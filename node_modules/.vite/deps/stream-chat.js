...
var channelHasReadEvents = (channel) => {
  var _a;
  const ownCapabilities = (_a = channel == null ? void 0 : channel.data) == null ? void 0 : _a.own_capabilities;
  return !(Array.isArray(ownCapabilities) && !ownCapabilities.includes("read-events"));
};
var channelTracksReadLocally = (channel) => !channelHasReadEvents(channel) && !!(channel == null ? void 0 : channel.getClient().options.isLocalUnreadCountEnabled);
var userHasReadReceipts = (client) => {
  var _a, _b, _c;
  return ((_c = (_b = (_a = client.user) == null ? void 0 : _a.privacy_settings) == null ? void 0 : _b.read_receipts) == null ? void 0 : _c.enabled) ?? true;
};
...
var deleteUserMessages = ({
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (messages[i].quoted_message && ((_c = (_b = message.quoted_message) == null ? void 0 : _b.user) == null ? void 0 : _c.id) === user.id) {
...
var getInitialMentions = (message) => {
  ...
  if ((_a = message.mentioned_roles) == null ? void 0 : _a.length) {
    mentions.push(
  ...
  if ((_b = message.mentioned_groups) == null ? void 0 : _b.length) {
    mentions.push(
  ...
  } else if ((_c = message.mentioned_group_ids) == null ? void 0 : _c.length) {
    mentions.push(
...
var UserGroupPaginator = class extends BasePaginator {
  constructor(client, options2) {
    ...
    this.query = async ({
      ...
      const cursor = decodeCursor((_a = this.cursor) == null ? void 0 : _a.next);
      const options3 = {
        limit: this.pageSize,
        ...this.teamId ? { team_id: this.teamId } : {},
        ...(cursor == null ? void 0 : cursor.id_gt) ? { id_gt: cursor.id_gt } : {},
        ...(cursor == null ? void 0 : cursor.created_at_gt) ? { created_at_gt: cursor.created_at_gt } : {}
      };
      const { user_groups: items } = await this.client.queryUserGroups(options3);
...
var ChannelMemberSearchSource = class extends BaseSearchSource {
  constructor(channel, options2, filterBuilderOptions = {}) {
    super(options2);
    this.type = "members";
    this.canExecuteQuery = (newSearchString) => {
      const hasNewSearchQuery = typeof newSearchString !== "undefined";
  ...
  async query(searchQuery) {
    ...
    const { members } = await this.channel.queryMembers(filters ?? {}, sort, options2);
...
var hasOwnCapability = (ownCapabilities, capability) => (ownCapabilities == null ? void 0 : ownCapabilities.includes(capability)) ?? false;
...
var DEFAULT_SUGGESTION_FACTORY_MAPPERS = {
  ...
  user_group: (value, { searchToken }) => {
    ...
    return {
       ...
       */
      memberCount: (_a = userGroup.members) == null ? void 0 : _a.length,
      mentionType: "user_group",
      name: userGroup.name,
      ...getTokenizedSuggestionDisplayName({
...
var MentionsSearchSource = class extends BaseSearchSource {
  constructor(channel, options2) {
    ...
    this.isMentionTypeAllowed = (mentionType) => {
      var _a;
      return getAllowedMentionTypesFromCapabilities((_a = this.channel.data) == null ? void 0 : _a.own_capabilities)[mentionType];
    };
    this.mapMentionSuggestion = (mentionType, value, searchToken = this.searchQuery) => {
      var _a;
      const mapper = ((_a = this.config.suggestionFactoryMappers) == null ? void 0 : _a[mentionType]) ?? DEFAULT_SUGGESTION_FACTORY_MAPPERS[mentionType];
      return mapper(value, {
        searchToken,
        source: this
      });
    ...
    this.getChannelTeam = () => {
      var _a;
      return (_a = this.channel.data) == null ? void 0 : _a.team;
    };
    ...
    this.getRoleMentionSuggestions = async (query) => {
      ...
      return [...(roles == null ? void 0 : roles.map((role) => role.name)) ?? []].sort((left, right) => left.localeCompare(right)).map((role) => this.toRoleMentionSuggestion(role, query));
    };
    this.searchMembersLocally = (searchQuery) => {
      ...
      return this.getMembersAndWatchers().filter((user) => {
        ...
        const maxDistance = 3;
        const trigger2 = this.config.trigger ?? "@";
        const lastDigits = textComposerText2.slice(-(maxDistance + 1)).includes(trigger2);
    ...
    this.queryUsers = async (searchQuery, offset = 0) => {
      const { filters, sort, options: options3 } = this.prepareQueryUsersParams(searchQuery, offset);
      const { users } = await this.client.queryUsers(filters, sort, options3);
      return users;
    };
    this.queryMembers = async (searchQuery, offset = 0) => {
      const { filters, sort, options: options3 } = this.prepareQueryMembersParams(
        searchQuery,
        offset
      );
    ...
    this.buildUserGroupSearchCursor = (items) => {
      if (items.length < this.pageSize) return void 0;
      const lastItem = items[items.length - 1];
      if (!(lastItem == null ? void 0 : lastItem.name)) return void 0;
      return JSON.stringify({
        id_gt: lastItem.id,
    ...
    this.getUserGroupSuggestionsPage = async (searchQuery, cursor) => {
      ...
      const options3 = {
        ...
        ...(userGroupCursor == null ? void 0 : userGroupCursor.id_gt) ? { id_gt: userGroupCursor.id_gt } : {},
        ...(userGroupCursor == null ? void 0 : userGroupCursor.name_gt) ? { name_gt: userGroupCursor.name_gt } : {}
      };
      const { user_groups } = await this.client.searchUserGroups(options3);
      return {
  ...
  async query(searchQuery) {
    ...
    const userResults = userResultsState.status === "fulfilled" ? userResultsState.value : {
      items: [],
      nextOffset: isFirstPage ? void 0 : previousUserPaginationState == null ? void 0 : previousUserPaginationState.nextOffset
    };
    ...
    handlers: {
      onChange: ({ state, next, complete, forward }) => {
        ...
        const mentionedUsersChanged = currentMentions.length !== state.mentionedUsers.length || currentMentions.some(
          (user, index) => {
            var _a2;
            return user.id !== ((_a2 = state.mentionedUsers[index]) == null ? void 0 : _a2.id);
          }
        );
        ...
        const triggerMatchesCommittedMention = !!triggerWithToken && /\s$/.test(textBeforeCursor) && (cursorJustInsertedAMention || (stateWithMentions.mentions ?? []).some((entity) => {
          ...
          return ((occurrences == null ? void 0 : occurrences.length) ?? 0) === 1;
        }));
        const triggerWasRemoved = !triggerWithToken || triggerWithToken.length < finalOptions.minChars;
        if (triggerWasRemoved || triggerMatchesCommittedMention) {
          const hasStaleSuggestions = ((_a = stateWithMentions.suggestions) == null ? void 0 : _a.trigger) === finalOptions.trigger;
...
var applyCommandValidatorOverride = (targetConfig, sourceConfig) => {
  var _a;
  const overrideValidator = (_a = sourceConfig == null ? void 0 : sourceConfig.commands) == null ? void 0 : _a.sendValidator;
...
var CustomDataManager = class {
  constructor({ composer, message }) {
    ...
    this.initState = ({ message: message2 } = {}) => {
      this.state.next(initState3({ composer: this.composer, message: message2 }));
    };
...
var _LinkPreviewsManager = class _LinkPreviewsManager2 {
  constructor({ composer, message }) {
    ...
    this.initState = ({ message: message2 } = {}) => {
      ...
      this.state.next(initState4({ message: this.enabled ? message2 : void 0 }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
...
var LocationComposer = class {
  constructor({ composer, message }) {
    this.initState = ({ message: message2 } = {}) => {
      this.state.next(initState5({ message: message2 }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
...
var Channel = class {
  ...
  markReadLocally() {
    ...
    const event = {
      ...
      cid: this.cid,
      created_at: (/* @__PURE__ */ new Date()).toISOString(),
      last_read_message_id: (_a = this.lastMessage()) == null ? void 0 : _a.id,
      team: (_b = this.data) == null ? void 0 : _b.team,
  ...
  _handleChannelEvent(event) {
    ...
    switch (event.type) {
      ...
      case "member.updated": {
        ...
        if (typeof currentUserId === "string" && typeof ((_o = memberCopy == null ? void 0 : memberCopy.user) == null ? void 0 : _o.id) === "string" && memberCopy.user.id === currentUserId) {
          channelState.membership = memberCopy;
        }
        break;
      }
      case "member.removed":
        if ((_p = event.user) == null ? void 0 : _p.id) {
          const newMembers = {
            ...channelState.members
          };
      ...
      case "notification.mark_unread": {
        const ownMessage = ((_q = event.user) == null ? void 0 : _q.id) === ((_r = this.getClient().user) == null ? void 0 : _r.id);
      ...
      case "channel.updated":
        if (event.channel) {
          const isFrozenChanged = ((_s = event.channel) == null ? void 0 : _s.frozen) !== void 0 && event.channel.frozen !== ((_t = channel.data) == null ? void 0 : _t.frozen);
          if (isFrozenChanged) {
            this.query({ state: false, messages: { limit: 0 }, watchers: { limit: 0 } });
          }
          const newChannelData = {
            ...event.channel,
            hidden: ((_u = event.channel) == null ? void 0 : _u.hidden) ?? ((_v = channel.data) == null ? void 0 : _v.hidden),
            own_capabilities: ((_w = event.channel) == null ? void 0 : _w.own_capabilities) ?? ((_x = channel.data) == null ? void 0 : _x.own_capabilities)
          };
      ...
      case "channel.visible":
        channel.data = {
          ...channel.data,
          blocked: !!((_z = event.channel) == null ? void 0 : _z.blocked),
          hidden: false
        };
        (_A = this.getClient().offlineDb) == null ? void 0 : _A.handleChannelVisibilityEvent({ event });
        break;
      case "user.banned":
        if (!((_B = event.user) == null ? void 0 : _B.id)) break;
        channelState.members[event.user.id] = {
          ...channelState.members[event.user.id] || {},
          shadow_banned: !!event.shadow,
          banned: !event.shadow,
          user: { ...((_C = channelState.members[event.user.id]) == null ? void 0 : _C.user) || {}, ...event.user }
        };
        break;
      case "user.unbanned":
...
var import_zlib = __toESM2(require_zlib());
...
var Moderation = class {
   ...
   * @returns
   */
  async check(entityType, entityID, entityCreatorID, moderationPayload, configKey, options2, testMode) {
    return await this.client.post(
...
var getResponsePaginationParams = ({
  ...
  const predefinedFilter = queryChannelsResponse == null ? void 0 : queryChannelsResponse.predefined_filter;
  if (!predefinedFilter) {
    return {};
  }
  return {
...
var ChannelManager = class extends WithSubscriptions {
  constructor({
    ...
    this.setChannels = (valueOrFactory) => {
      ...
      const {
        channels,
        pagination: { filters, options: options3, sort }
      } = this.state.getLatestValue();
      (_a = this.client.offlineDb) == null ? void 0 : _a.executeQuerySafely(
  ...
  clear() {
    if (this.dispose) {
      this.map.forEach((entry, key) => {
        var _a;
        return (_a = this.dispose) == null ? void 0 : _a.call(this, key, entry);
      });
    }
    this.map.clear();
    this.keys = [];
...
var StreamChat = class _StreamChat {
    ...
    this.getUserAgent = () => {
      ...
      if (!this.cachedUserAgent) {
        ...
        this.cachedUserAgent = [
          ...
          }).filter(([, value]) => value && value.length > 0).map(([key2, value]) => `${key2}=${value}`)
        ].join("|");
      }
  ...
  _handleClientEvent(event) {
    ...
    if (event.type === "notification.removed_from_channel" && event.cid) {
      const { cid } = event;
      (_d = this.activeChannels[cid]) == null ? void 0 : _d._disconnect();
      postListenerCallbacks.push(() => {
        delete this.activeChannels[cid];
      });
    }
   ...
   */
  async queryUserGroups(options2 = {}) {
    return await this.get(this.baseURL + "/usergroups", options2);
  }
  /**
   ...
   */
  async createUserGroup(options2) {
   ...
   */
  async getUserGroup(id, options2 = {}) {
   ...
   */
  async searchUserGroups(options2) {
    return await this.get(
   ...
   */
  async updateUserGroup(id, options2) {
   ...
   */
  async deleteUserGroup(id, options2 = {}) {
   ...
   */
  async addUserGroupMembers(id, options2) {
    return await this.post(
   ...
   */
  async removeUserGroupMembers(id, options2) {
    return await this.post(
   ...
   */
  async queryChannelsRequestWithResponse(filterConditions, sort = [], options2 = {}) {
    const defaultOptions = {
      state: true,
      watch: true,
      presence: false
  ...
  async queryChannels(filterConditions, sort = [], options2 = {}, stateOptions = {}) {
    ...
    const hydratedChannels = this.hydrateActiveChannels(channels, stateOptions, options2);
   ...
   * @returns {Promise<APIResponse>}
   */
  searchRoles(options2) {
    return this.get(`${this.baseURL}/roles/search`, options2);
  }
          ...
          if (cid && client2.user && client2.user.id !== (user == null ? void 0 : user.id)) {
            ...
            if (channel && (channelHasReadEvents(channel) || tracksReadLocally)) {
              ...
              const upsertReadsQueries = await this.upsertReads({
                ...
                reads: [
                  {
                    last_read: ((ownReads == null ? void 0 : ownReads.last_read) ?? /* @__PURE__ */ new Date(0)).toISOString(),
                    last_read_message_id: ownReads == null ? void 0 : ownReads.last_read_message_id,
                    unread_messages: unreadCount,
                    user: client2.user
                  }
                ]
      ...
      if (channel && ownUser) {
        ...
        if (activeChannel && (channelHasReadEvents(activeChannel) || tracksReadLocally)) {
          ...
          const upsertReadQueries = await this.upsertReads({
            ...
            reads: [
              {
                last_read: ((ownReads == null ? void 0 : ownReads.last_read) ?? /* @__PURE__ */ new Date(0)).toString(),
                last_read_message_id: ownReads == null ? void 0 : ownReads.last_read_message_id,
                unread_messages: unreadCount,
                user: ownUser
              }
