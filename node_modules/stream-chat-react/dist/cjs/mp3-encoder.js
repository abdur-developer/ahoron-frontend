...
//#region src/plugins/encoders/mp3.ts
...
var float32ArrayToInt16Array = (float32Arr) => {
	...
	for (let i = 0; i < float32Arr.length; i++) {
		...
		int16Arr[i] = Math.round(Math.max(-1, Math.min(1, float32Value)) * 32767);
	}
	return int16Arr;
};
var splitDataByChannel = (audioBuffer) => Array.from({ length: audioBuffer.numberOfChannels }, (_, i) => audioBuffer.getChannelData(i)).map(float32ArrayToInt16Array);
async function encodeToMp3(file, sampleRate) {
	const lameJs = await import("@breezystack/lamejs");
	const audioBuffer = await require_audioProcessing.renderAudio(await require_audioProcessing.toAudioBuffer(file), sampleRate);
...
//#endregion
exports.encodeToMp3 = encodeToMp3;
