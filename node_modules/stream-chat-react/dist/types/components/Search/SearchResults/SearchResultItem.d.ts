import type { ComponentType } from 'react';
import type { Channel, MessageResponse, User } from 'stream-chat';
export type ChannelSearchResultItemProps = {
    item: Channel;
};
export declare const ChannelSearchResultItem: ({ item }: ChannelSearchResultItemProps) => import("react/jsx-runtime").JSX.Element;
export type ChannelByMessageSearchResultItemProps = {
    item: MessageResponse;
};
export declare const MessageSearchResultItem: ({ item, }: ChannelByMessageSearchResultItemProps) => import("react/jsx-runtime").JSX.Element | null;
export type UserSearchResultItemProps = {
    item: User;
};
export declare const UserSearchResultItem: ({ item }: UserSearchResultItemProps) => import("react/jsx-runtime").JSX.Element;
export type SearchResultItemComponents = Record<string, ComponentType<{
    item: any;
}>>;
export declare const DefaultSearchResultItems: SearchResultItemComponents;
//# sourceMappingURL=SearchResultItem.d.ts.map